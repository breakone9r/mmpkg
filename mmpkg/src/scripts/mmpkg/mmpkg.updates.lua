function mmpkg.checkforUpdate() noop() end

function mmpkg.doUpdate() noop() end

function mmpkg.mapdownloaded(_, filename)
    if not filename:find("mm_map.dat.new", 1, true) then return end
    ...
    if roomExists(gmcp.room.info.num) then
        ...
        cecho(
            "<white>Unknown Room. Please move to a known room (And then map this one!)")
    end
end
...
function mmpkg.updateMap()
    if mmpkg.downloadconfirmed then
        ...
        tempTimer(3, function() mmpkg.downloadconfirmed = nil end)
    end
end
