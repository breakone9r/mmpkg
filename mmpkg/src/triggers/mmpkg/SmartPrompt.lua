...
if not mmpkg.conf.smartprompt or not mmpkg.myAffects then
  ...
  if table.contains(roomflags, "half-pk-damage") then
    roomflag = roomflag.."<grey>[HalfDmgPk]"
  end
  if table.contains(roomflags, "double-pk-damage") then
    roomflag = roomflag.."<grey>[DblDmgPk]"
  end
  if table.contains(roomflags, "no-form") then
    roomflag = roomflag.."<grey>[NoForm]"
  end
  if table.contains(roomflags, "no-trap") then
    roomflag = roomflag.."<grey>[NoTrap]"
  end
  if table.contains(roomflags, "anti-magic") then
    roomflag = roomflag.."<grey>[NoMag]"
  end
  if table.contains(roomflags, "silence") then
    roomflag = roomflag.."<grey>[Silent]"
  end
  if table.contains(roomflags, "icy-conditions") then
    roomflag = roomflag.."<cyan>[Icy]"
  end
  if table.contains(roomflags, "no-npcs") then
    roomflag = roomflag.."<grey>[NoNPC]"
  end
  if table.contains(roomflags, "no-recall") then
    roomflag = roomflag.."<grey>[NoRecall]"
  end
